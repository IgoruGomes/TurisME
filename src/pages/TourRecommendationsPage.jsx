...
// Componente unificado para imagens de lugares
...
setLoading(true);
try {
const { data, error } = await fetch('/api/fetch-places-for-tour', { // ajustar para Supabase function se necessário
method: 'POST',
body: JSON.stringify({ categories: [category], lat: coords.lat, lon: coords.lon }),
});
const result = await data.json();
if (error) throw error;
const filtered = (result[category] || []).filter(p => p.place_id !== currentPlace.place_id).slice(0, 5);
...
<PlaceImage photo_reference={alt.photos?.[0]?.photo_reference} alt={alt.name} className="w-16 h-16 rounded-md object-cover" maxWidth={200} />
<div className="flex-grow">
<p className="font-semibold">{alt.name}</p>
<p className="text-sm text-gray-500 line-clamp-1">{alt.vicinity}</p>
</div>
</div>
))}
</div>
...
setLoading(true);
try {
const { data } = await fetch('/api/fetch-places', { // ajustar para Supabase function se necessário
method: 'POST',
body: JSON.stringify({ query: searchQuery, lat: coords?.lat, lon: coords?.lon }),
});
const result = await data.json();
...
<input type="search" placeholder="Buscar local..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full pl-3 pr-4 py-2 rounded-full bg-gray-100 border-transparent focus:ring-2 focus:ring-blue-500"/>
...
<PlaceImage photo_reference={place.photos?.[0]?.photo_reference} alt={place.name} className="w-16 h-16 rounded-md object-cover" maxWidth={200} />
<div className="flex-grow">
<p className="font-semibold">{place.name}</p>
<p className="text-sm text-gray-500 line-clamp-1">{place.vicinity}</p>
</div>
</div>
))}
</div>
...
return tourRecommendations.reduce((total, place) => total + (priceAverages[place.price_level] || 0), 0);
}, [tourRecommendations]);
...
const handleCancel = () => { clearTour(); navigate('/home'); };
const handleStart = () => { startActiveTour(); navigate('/active-tour'); };
const handleAddPlace = (place) => { addRecommendation(place); };
const handleOpenSwapModal = (place) => {
const category = tourSettings.categories.find(c => place.types?.some(t => t.replace(/_/g, ' ').toLowerCase().includes(c.toLowerCase().split(' ')[0]))) || tourSettings.categories[0];
...
<header className="sticky top-0 bg-white/80 backdrop-blur-sm z-10 p-4 border-b flex justify-between items-center max-w-4xl mx-auto w-full">
...
<h1 className="text-xl font-bold">Seu Tour Recomendado</h1>
    ...
    <main className="flex-grow p-4 overflow-y-auto max-w-4xl mx-auto space-y-4">
      {tourRecommendations.length > 0 ? tourRecommendations.map((place, index) => (
        <motion.div key={place.place_id + index} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.1 }} className="bg-white rounded-xl shadow-md overflow-hidden flex items-start gap-3 p-3">
          <div className="relative w-24 h-24 sm:w-32 sm:h-32 flex-shrink-0">
            <PlaceImage photo_reference={place.photos?.[0]?.photo_reference} alt={place.name} className="w-full h-full object-cover rounded-lg" />
    ...
    <footer className="sticky bottom-0 bg-white border-t p-4 max-w-4xl mx-auto w-full">
